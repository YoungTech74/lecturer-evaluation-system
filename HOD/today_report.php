...
if(isset($_SESSION['HOD_user'])){
    $today = date("Y-m-d");
    $count1 = 0;
    $getRecord = mysqli_query($conn, "SELECT * FROM reports WHERE status = 'Inactive' AND date_created = '$today' AND department = '$_SESSION[HOD_dep]' ORDER BY id DESC");
...
<div class="col-lg-12">
            ...
            <h3 class="float-left p-2">Today's Lectures</h3>
                ...
                <?php
                   ...
                   <div class="col-md-5 mb-2 ms-2 border-righ" style="box-shadow: 5px 5px 8px grey, -5px -1px 8px grey; border-radius: 10px;">
                        ...
                        <label for="">Time Out : </label><input type="text" value="<?php echo $row['time_out']; ?>" readonly><br><br>
			...
			<table class="table tabe-hover table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td data-label="Time Left"><b><?php echo $search['leave_time'] ?></b></td>
						<td data-label="Action"><b><?php echo $search['date_created'] ?></b></td>

						<td data-label="Action" class="text-center">
		                      <a class="dropdown-item " href="./remark.php?remark_id=<?php echo $search['id'] ?>"> <span class="btn btn-xs btn btn-info">Give Remark</span></a>
						</td>
					</tr>	
				<?php endwhile; ?>
				</tbody>
    ...
    //------------------------------ if the search button is not clicked -------------------------
        ...
        $count2 = 0;
        $sql = mysqli_query($conn, "SELECT * FROM reports WHERE status = 'Active' && department = '$_SESSION[HOD_dep]' ORDER BY id DESC LIMIT 5");
       if($sql){?>
       <head>
        <link rel="stylesheet" href="../style.css">
       </head>
			...
			<table class="table tabe-hover table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td data-label="Left Time"><b><?php echo $row2['leave_time'] ?></b></td>
						<td data-label="Date Created"><b><?php echo $row2['date_created'] ?></b></td>
						<td data-label="Action" class="text-center">
		                      <a class="dropdown-item " href="./remark.php?remark_id=<?php echo $search['id'] ?>"> <span class="btn btn-xs btn btn-info">Give Remark</span></a>
						</td>
					</tr>	
